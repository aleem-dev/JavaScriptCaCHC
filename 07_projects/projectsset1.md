...
form.addEventListener("submit", function(e){
  ...
  if(height === 0 || height < 0 || isNaN(height)){
    ...
    results.innerHTML = `<span>${bmi}</span>`
    if(bmi < 18.6){
      highlightguide("under",bmi)
    }else if (bmi === 18.6 || bmi < 24.9){
      highlightguide("normal", bmi)
    }else if(bmi >= 24.9){
      highlightguide("over", bmi)
    }
  }
const highlightguide = (id, bmi) => {
    const buff = `=>  `+ document.querySelector(`#${id}`).innerHTML
    document.querySelector(`#results`).innerHTML = `${bmi} `+ `<span style="color: orange;">${buff}</span>`
